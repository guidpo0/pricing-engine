...
@pytest.mark.asyncio
async def test_get_crypto_quote_success():
...
async def test_get_crypto_quote_rate_limit_fallback():
    ...
    assert result["price"] == 64000.00 # Returned from fallback

@pytest.mark.asyncio
async def test_get_crypto_quote_rate_limit_no_fallback():
    mock_response = MagicMock()
    mock_response.status_code = 429
    
    with patch("httpx.AsyncClient.get", return_value=mock_response):
        with patch("app.services.crypto_market_service.add_crypto_slug"):
            with pytest.raises(ValueError, match="Rate limit exceeded for bitcoin on first fetch \\(no cache available\\). Try again later."):
